...
# ----------------------------------------------------------------------------
# NOTE: curve FITTING has been split OUT of this plugin.  The plugin only writes
# the raw concentration_history CSV + summary (data output).  The UMAT card
# (PROPS 40-54) is fitted from that CSV by the standalone, modular fitter:
#     D:\Desktop\SCI_3\program\fit_UMAT_curves.py
# ----------------------------------------------------------------------------
...
def extract_concentration_factors(odb, step_name, instance_name, rp_history,
    ...
    for fr in range(nfr):
        ...
        mis_sum = 0.0
        mis_pk = 0.0
        mis_sq_sum = 0.0          # rev.20 Route A: IVOL-weighted Mises^2 (second moment)
        ...
        # rev.20 Route A (XKYLD, principled): volume-weighted high quantile of the
        # matrix Mises field.  Yield onset is LOCAL-driven so the mean field (and
        # the mild 2nd-moment ratio ~1.05) underestimate it; the single max
        # (k_yield ~5.7) is singularity-/mesh-dominated.  The P90-P95 band is the
        # representative stress of the most-loaded 5-10% of matrix VOLUME -- the
        # part that yields first and drives the macro knee -- and is mesh-robust.
        mis_q90 = _wquantile(vm_pairs, 0.90)
        mis_q95 = _wquantile(vm_pairs, 0.95)
        # rev.20 Route A: second-moment (RMS) of matrix Mises.  The Doghri/MFH
        # intra-phase stress heterogeneity ratio  sqrt(<Mises^2>)/<Mises>  is the
        # physically correct YIELD-onset amplifier (~1.1-1.3), replacing the
        # geometric peak/avg ratio (k_yield ~5.7) that yields the element far too
        # early.  DATA OUTPUT ONLY here; fit_UMAT_curves.py consumes the column.
        ...
        #  cohesive ELEMENTS -> peak direct stress S over the interface set
        #    (in-plane components ~0 for traction-separation, so max of the 3
        #     direct components robustly picks the normal traction regardless
        #     of which local axis Abaqus assigned to S33);
        #  cohesive CONTACT  -> fall back to CSTRESS.
        # rev.20 XKINT Route A: interface normal traction.
        #   tn_pk  = MAX direct stress over the interface set (geometric hot spot),
        #            kept for reference / back-compat.
        #   tn_avg = IVOL(area)-weighted AVERAGE interface normal traction.  The
        #            cohesive layer has the SAME peak/avg heterogeneity disease as
        #            k_yield: the MAX iface Tn reaches TN0=50 at a single hot spot
        #            ~eps 0.0018, firing whole-element debond far too early (run 74:
        #            collapse @ eps~0.0017).  The AREA-AVERAGED traction is the
        #            physically correct whole-element debond driver, so XKINT is
        #            built from tn_avg (mirrors the XKYLD second-moment fix).
        #            Only tensile (cand > 0) contributions are accumulated.
        tn_pk = 0.0
    ...
    for r in rows:
        ...
        # rev.20 Route A: second-moment yield amplifier  sqrt(<Mises^2>)/<Mises>.
        # Kept for reference (it is too diluted: ~1.05).
        ...
        # rev.20 Route A (XKINT): interface-AVERAGE debond amplifier
        # <Tn_iface> / <sigma_m_transverse>.  Replaces the max(Tn)/avg ratio,
        # which fired at a single geometric hot spot (~eps 0.0018) and caused
        # premature brittle whole-element debond.  k_int (peak-based) kept above
        # for reference; the fitter prefers XKINT_avg for the debond driver.
    ...
    # rev.20 Route A: RVE-pinned yield-knee strain eps_y = first frame where the
    ...
    # rev.20 Route A (XKINT): pin the debond eps_y to the MACRO stress peak strain
    ...
    # rev.20: parameter FITTING is intentionally NOT performed here.  The plugin
    # only OUTPUTS data -- the concentration_history CSV + summary written above.
    # Fit the UMAT card (PROPS 40-54) from that CSV with the standalone, modular
    # fitter (data output and fitting are kept separate):
    #     D:\Desktop\SCI_3\program\fit_UMAT_curves.py
...
def rve_multiaxial_analysis_complete(part, inst, meshsens, theta_deg, epsilon_x, epsilon_y, epsilon_z,
    ...
    for T in range(1):
        ...
        # softening localization / snap-back that stalled the pure-static
        # solver at the damage band (the energy check showed ALLSD blowing
        # up only in the terminal stall), so the descending branch can be
        # traced further.  No *STATIC stabilize damping here; regularization
        ...
        # ============================================================
        # ---- DISABLED 2026-06-22 (reverted to *Static; quasi-static dynamics
        #      retired).  Quasi-static implicit dynamics is unsuitable for a
        #      PBC RVE driven through MASSLESS reference-point control nodes:
        #      the M/(beta*dt^2) inertia term is ill-defined on those DOFs
        #      (increment-1 NaN), and the inertia regularization pollutes the
        #      stress-strain curve that Melro (2013) defines as a *static*
        #      result.  Softening is now handled by *Static adaptive energy
        #      stabilization (below), mirroring PBC_UDFRP_Elastoplastic.py.
        #      To restore: uncomment this block, re-comment the StaticStep block.
        ...
        # ------------------------------------------------------------
        # ACTIVE STATIC STEP (re-enabled 2026-06-22).  Adaptive energy
        # stabilization (DISSIPATED_ENERGY_FRACTION, DEF=2e-4, ALLSDTOL=0.05)
        # damps the damage-softening localization without physical inertia,
        # matching the no-interface kernel.  IMPORTANT: with no inertia term,
        # the UMAT must run IDMTAN=0 (secant); IDMTAN=1 (consistent tangent)
        # is indefinite in softening and will diverge under *Static.
        # ------------------------------------------------------------
        ...
        print('[StaticStep] Solution controls: R_n=0.02, C_n=1.0, '
              'discontinuous=ON (I0=8, IR=10), IC=30, I_A=10.')
        '''
        # For UMAT
        ...
        if error==False:
            ...
            for layerIndex, layerNodes in enumerate(interfaceDuplicateLayers, start=1):
                ...
                print('[Interface] Added elastoplastic PBC equations for duplicate node layer %s (%s nodes).' % (layerIndex, len(layerNodes)))
