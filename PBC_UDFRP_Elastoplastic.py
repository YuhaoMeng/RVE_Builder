...
def create_csv_writer(filename, fieldnames):
    ...
    try:
        # Open in binary (Py2) / newline='' (Py3) so the csv module does not add a
        # stray '\r' on Windows (text-mode '\n' -> '\r\r\n'), which would otherwise
        # leave a blank row between every data row.
        import sys as _sys
...
# NO-CZM kernel: interface is OFF, so there are no debond_* fields; the "onset"
# here is the macro-stress peak (matrix-damage onset), not interface debond.
_CONC_SUMMARY_DOC = [
    ...
    ('SCFt_at_onset',         '★PROPS(29) SCFt: matrix transverse stress peak/mean @peak; transverse-tension stress concentration'),
    ...
    ('SCF_loc_at_onset',      'peak frame SCF_loc(RVE) = matrix mean / macro; used to compute alpha'),
    ...
    ('onset_frame',           'frame index of the macro-stress peak (matrix damage onset)'),
    ('onset_macro',           'macro stress at peak (MPa); approximately the green-line peak'),
    ('onset_iface_Tn',        'no-interface model: always 0'),
    ('onset_by',              'peak criterion: macro_stress_peak (no interface)'),
]
...
def write_summary_txt(path, summary):
    ...
    try:
        ...
        f = open(path, 'w')
        f.write('# RVE stress-concentration factor summary  (UMAT calibration, NO-CZM)\n')
        f.write('# one variable per line;  name = value   # meaning\n')
        f.write('# the two rows marked PROPS(..) and ★ are the stress-concentration factors to enter in the material card\n')
        f.write('# ' + '-' * 64 + '\n')
...
FIELD_OUTPUT_NUM_INTERVALS = 0   # 0 = every increment (frequency=1, full
                                 # output).  User requires the extraction
                                 # pipeline to see ALL frames -- do not
                                 # reduce field output again.
...
def extract_concentration_factors(odb, step_name, instance_name, rp_history,
    ...
    def _find_eset(name):
        wl = name.strip().upper()
        for src in (instance.elementSets, odb.rootAssembly.elementSets):
            try:
                for k in src.keys():
                    if k.upper() == wl:
                        return src[k]
    ...
    rd = rows[idx_deb]
    # --- EVENT-MATCHED ("effective trigger") factors (no interface here) -------
    # factor = micro-threshold / (matrix stress @ event).  Card value =
    # eff_trigger * alpha, alpha = SCF_loc_RVE / SCF_loc_UMAT (STATEV34/macro on
    # the single-element ODB) -- the UMAT's SIGML localizes stronger than the RVE.
    ...
    summary = {
        ...
        'SCF_loc_at_onset': rd.get('SCF_loc', 0.0),
        'alpha_hint': 'card = eff_trigger * SCF_loc_RVE / SCF_loc_UMAT(STATEV34/macro)',
        'onset_frame': rd['Frame'],
        'onset_macro': rd[mac_key],
        'onset_iface_Tn': rd['Iface_Tn_pk'],
        'onset_by': 'interface_traction' if has_iface else 'macro_stress_peak',
    }
...
def calculate_true_stress_strain_from_rp(rp_results, loading_type, L, H, W):
    ...
    for frame in sorted(frame_data.keys()):
        ...
        if loading_type == 'E11':  # tension in X
            ...
            result_dict.update({
                'True_Shear_Strain': true_shear_strain_tensor,
...
def save_true_stress_strain(true_results, output_dir, modelName, loading_type, theta_deg=0):
    ...
    engineering shear) is unambiguous in the header:
      * Normal (E11 / E22 / E33):
        ...
        The trailing *_Abs columns repeat the same values with abs() applied, so
        compression cases (negative strain / stress) can be plotted in the upper
        half plane directly without post-processing in the spreadsheet.
      * Shear (G12 / G13 / G23):
            Frame, Time, True_Shear_Strain, True_Shear_Stress
        where True_Shear_Strain = ln(1 + eps_ij) * sign(eps_ij), eps_ij = gamma_eng / 2
        is the tensor true shear strain (multiply by 2 to recover engineering gamma).
    ...
    is_shear = any([k in sample_row for k in ('True_Shear_Stress', 'True_Shear_Strain')])
...
def rve_multiaxial_analysis_complete(part, inst, meshsens, theta_deg, epsilon_x, epsilon_y, epsilon_z,
    ...
    for T in range(1):
        ...
        # Matches the manual CAE workflow:
        #   Specify ON;
        #   Field Equations tab > C_n^a relaxed from 0.01 to 1.0, and
        #     R_n^a relaxed from 0.005 to 0.02: in the damage-softening
        #     band the residual stalls at a noise floor of ~1-2% of the
        #     (tiny) average flux -- mechanically converged, but the
        #     0.5% default kept declaring "solution diverging" and the
        #     increments collapsed to dt~1e-6.  2% passes the converged
        #     states and still catches real divergence.
        #   Time Incrementation tab > Discontinuous analysis (I0=8, IR=10);
        #   More... > IC = 30 (default was 16),
        #               I_A = 10 (default was 5) -- allow more cut-back
        #               attempts per increment before the whole increment is
        #               declared a failure. Plastic-softening hot spots often
        #               need 6-8 cut-backs to settle; 5 is too tight and
        #               leads to a full-job abort. The remaining slots in
        #               both tuples stay at their Abaqus defaults.
        # displacementField tuple positions:
        #   1: R_n^a   (force residual)          -> 0.02   (relaxed from 0.005)
        #   2: C_n^a   (displacement correction) -> 1.0    (relaxed from 0.01)
        #   3-11: defaults
        # timeIncrementation tuple positions:
        #   1: I0  =  8 (discontinuous-analysis equilibrium iterations)
        #   2: IR  = 10 (discontinuous-analysis residuals check window)
        #   3: IP  =  9 (default)
        #   4: IC  = 30 (max equilibrium iterations; default was 16)
        #   5: IL  = 10 (default)
        #   6: IG  =  4 (default)
        #   7: IS  = 12 (default)
        #   8: I_A = 10 (max cut-back attempts per increment; default was 5)
        #   9-11: defaults
        ...
        if error==False:
            ...
            # Field output: evenly-spaced frames (FIELD_OUTPUT_NUM_INTERVALS) to
            # keep the ODB small and avoid serial I/O dominating wall-clock.
            # Set FIELD_OUTPUT_NUM_INTERVALS=0 to restore every-increment output.
