...
def feasypbc(part,inst,meshsens,K11,K22,K33,CPU,onlyPBC, intemp, fntemp):
    ...
    for T in (range(1)):
        ...
        # ---- Element-family guard (THERMAL conductivity analysis) ----
        # Abort early if this model carries a COHESIVE debonding seam, i.e. it
        # was built for a MECHANICAL interface analysis.  The cohesive seam set
        # is the reliable marker (insert_cohesive_seam always creates it).
        for _pname in mdb.models[modelName].parts.keys():
            _ipart = mdb.models[modelName].parts[_pname]
            _is_coh = ('CohesiveSeam-1-Elements' in _ipart.sets.keys())
            if not _is_coh and len(_ipart.elements):
                _is_coh = str(_ipart.elements[0].type).upper()[:3] == 'COH'
            if _is_coh:
                raise ValueError(
                    "Thermal conductivity analysis ABORTED: model '%s' part "
                    "'%s' has a cohesive (mechanical debonding) interface. "
                    "Run the mechanical module, or build a thermal interface "
                    "(contact pair + gap conductance) instead."
                    % (modelName, _pname))
